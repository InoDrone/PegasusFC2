    ...
    namespace timer {
      ...
      class Timer {
          public:
            ...
            static void setMode(Channel channel, Mode);
            ...
            static void enableInterrupt(uint8_t priority);
      ...
      void Timer<A>::setMode(Channel channel, Mode mode) {
        ...
        switch(mode) {
          case MODE_IC: // Input compare
            ...
            switch(channel) {
              case CHANNEL1:
                TIM_ITConfig(reg, TIM_IT_CC1, ENABLE);
                break;
              case CHANNEL2:
                TIM_ITConfig(reg, TIM_IT_CC2, ENABLE);
                break;
              case CHANNEL3:
                TIM_ITConfig(reg, TIM_IT_CC3, ENABLE);
                break;
              case CHANNEL4:
                TIM_ITConfig(reg, TIM_IT_CC4, ENABLE);
                break;
            }
      ...
      template<Address A>
      void Timer<A>::enableInterrupt(uint8_t priority) {

        TIM_TypeDef* tim = reinterpret_cast<TIM_TypeDef*>(A);
        IRQn irq;
